...
import { useZxWebStore } from '@/stores/zxWebStore'
...
function ProviderCard({ providerId, providerName }: { providerId: string; providerName: string }) {
  const accounts = useZxWebStore((s) => s.accounts)
  const startInAppLogin = useZxWebStore((s) => s.startInAppLogin)
  const deleteAccount = useZxWebStore((s) => s.deleteAccount)
  const loginInProgress = useZxWebStore((s) => s.loginInProgress)
  const loginProviderId = useZxWebStore((s) => s.loginProviderId)
...
function ProxyStatusBar() {
  const proxyStatus = useZxWebStore((s) => s.proxyStatus)
  const loadProxyStatus = useZxWebStore((s) => s.loadProxyStatus)
  const restartProxy = useZxWebStore((s) => s.restartProxy)
...
export const ZxWebSettings = memo(function ZxWebSettings() {
  const loadProviders = useZxWebStore((s) => s.loadProviders)
  const loadAccounts = useZxWebStore((s) => s.loadAccounts)
  const fetchModels = useZxWebStore((s) => s.fetchModels)
  const error = useZxWebStore((s) => s.error)
