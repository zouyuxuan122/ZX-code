...
vi.mock('../../../../main/chat2api/proxy/forwarder', () => ({
  requestForwarder: {
    forwardChatCompletion: vi.fn(),
  },
}))

vi.mock('../../../../main/chat2api/proxy/loadbalancer', () => ({
  loadBalancer: {
    selectAccount: vi.fn(),
...
vi.mock('../../../../main/chat2api/proxy/status', () => ({
  proxyStatusManager: {
...
vi.mock('../../../../main/chat2api/proxy/modelMapper', () => ({
  modelMapper: {
...
vi.mock('../../../../main/chat2api/store/store', () => ({
  storeManager: {
    getConfig: vi.fn(() => ({
      retryCount: 0,
...
vi.mock('../../../../main/chat2api/proxy/stream', () => ({
  streamHandler: {
    createTransformStream: vi.fn(),
  },
}))

vi.mock('../../../../main/chat2api/proxy/utils/toolFormatConverter', () => ({
  isAnthropicToolFormat: vi.fn(() => false),
  transformResponseToAnthropic: vi.fn(),
  transformChunkToAnthropic: vi.fn(),
}))

import router from '../../../../main/chat2api/proxy/routes/chat'
import { requestForwarder } from '../../../../main/chat2api/proxy/forwarder'
import { loadBalancer } from '../../../../main/chat2api/proxy/loadbalancer'
