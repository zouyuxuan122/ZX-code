...
| **图形化桌面 UI** | 无边框窗口 + 九宫格布局 + 深浅主题 + 动画 | 终端 CLI | 终端 CLI | 终端 CLI |
| **3D 桌宠 & AI 伴侣** | Live2D / VRM / SVG | 无 | 无 | 无 |
| **九宫格多面板工作台** | 9 格自由拖拽布局 | 无 | 无 | 无 |
...
| **本地优先 & 数据自主** | SQLite 本地存储 | 本地 | 本地 + 云端 | 本地 |
| **MCP 协议扩展** | 支持 | 支持 | 支持 | 支持 |
| **开源免费** | GPL-3.0 | MIT 开源 | 闭源付费 | Apache 2.0 开源 |

> **核心差异**：ZX-Code 是唯一具备「免 Key 国产大模型接入 + 图形化桌面 UI + 3D 桌宠陪伴 + 九宫格工作台 + 长期记忆」的桌面 AI 编程应用。相比 OpenCode、Claude Code、Codex 等终端 CLI 工具，ZX-Code 提供完整的桌面级交互体验。
...
| **Graphical desktop UI** | Frameless window + 9-grid + dark/light + animations | Terminal CLI | Terminal CLI | Terminal CLI |
| **3D desktop pet & AI companion** | Live2D / VRM / SVG | None | None | None |
| **9-grid multi-panel workspace** | 9 drag-and-drop panels | None | None | None |
...
| **Local-first & data ownership** | SQLite local storage | Local | Local + Cloud | Local |
| **MCP protocol** | Supported | Supported | Supported | Supported |
| **Open source & free** | GPL-3.0 | MIT Open | Closed/Paid | Apache 2.0 Open |

> **Key differentiator**: ZX-Code is the only AI coding app that combines "no-key Chinese LLM access + graphical desktop UI + 3D desktop pet companion + 9-grid workspace + long-term memory" in one package. Unlike terminal-based tools like OpenCode, Claude Code, and Codex, ZX-Code delivers a full desktop-grade interactive experience.
